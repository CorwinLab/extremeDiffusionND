...
def executeMoves(occupancy, i, j, rng, distribution, PDF, params=None):
    ...
    if PDF:  # if doing PDF then multiply by biases
        occupancy = occupancy.astype(float)  # since the PDF requires floats but the agents require ints, cast correctly
...
def evolve2DLattice(occupancy, maxT, distribution, params, PDF, startT=1,
    ...
    for t in range(startT, maxT):
        ...
        occupancy = executeMoves(occupancy, i, j, rng, distribution, PDF, params).astype(np.quad)
        yield t, occupancy


#TODO: rename this
#TODO: seperate PDF evolution from agent evolution....
def generateFirstArrivalTime(occupancy, maxT, distribution, params, PDF, startT=1,
    ...
    if PDF and occupancy != 1:
        print("Warning: Evolving a PDF with N!= 1 won't be normalized.")
    ...
    if np.isscalar(occupancy):
        ...
        occupancy[L, L] = 1
        if PDF:
            CDF = np.copy(occupancy).astype(np.quad) #introduce cumulative as npqaud
    ...
    listofNs = [100, 1000, 100000, 100000000, 1e10]
    for t, occ in evolve2DLattice(occupancy, maxT, distribution, params, PDF,boundary=absorbingBoundary):
        ...
        if PDF:
            ...
            if t in listofTimes:
                #stats[t] = {N: getPDFContour(occ,N) for N in listofNs} #update dictionary
                # p, a, r, d, d2, 1/N,
                tempstats = np.expand_dims(np.array([getContourRoughness(CDF, N) for N in listofNs]),axis=0)
                stats = np.vstack((stats,tempstats))
        elif not PDF: # i guess this is the same as else:
            if t == startT+1:
                stats = np.array(getTArrivalRoughness(tArrival,t))
            if t in listofTimes:
...
def getContourRoughness(Array, N):
    ...
    if np.all(mask == False):
        ...
        print(f"N = {N} and max PDF val = ",np.max(Array))
    #TODO: with PDF 1/N contour, could be getting Nones
